...
public class GradesParserLoader extends AsyncTaskLoader<String> {
	...
	// Constants defined for each different grade the student is taking, usually
	// only one
	...
	public static final String nStudentNip = "nip"; //Unknown value detected during tests
	...
	public String loadInBackground() {
		...
		try {
			...
			if (studentId == -1) {
				...
				studentValues.put(GradesContract.StudentsTable.COL_ST_NIF,
						student.getString(nStudentNif));
				String studentNip;
				if (student.has(nStudentNip)) {
					studentNip = student.getString(nStudentNip);
				}else{
					studentNip = "0";
				}
				studentValues.put(GradesContract.StudentsTable.COL_ST_NIP,
						studentNip);
				studentValues.put(GradesContract.StudentsTable.COL_ST_NAME,
						student.getString(nStudentName));
				studentValues.put(
						GradesContract.StudentsTable.COL_ST_SURNAME_1,
						student.getString(nStudentSurname1));
				studentValues.put(
						GradesContract.StudentsTable.COL_ST_SURNAME_2,
						student.getString(nStudentSurname2));
			...
			for (int resultsCounter = 0; resultsCounter < planList.length(); resultsCounter++) {
				...
				if (planId == -1) {
					...
					courseValues.put(GradesContract.CoursesTable.COL_CO_NAME,
							planName);
					courseValues.put(GradesContract.CoursesTable.COL_CO_CENTER,
							plan.getString(nPlanCenter));
					courseValues.put(
							GradesContract.CoursesTable.COL_CO_STUDIES,
							plan.getString(nPlanStudies));
					courseValues.put(
							GradesContract.CoursesTable.COL_CO_TOTAL_CREDITS,
							plan.getString(nPlanTotalCredits));
					courseValues.put(
							GradesContract.CoursesTable.COL_CO_PASSED_CREDITS,
							plan.getString(nPlanPassedCredits));
					courseValues.put(
							GradesContract.CoursesTable.COL_CO_LANGUAGE,
							language);

					// We will use a Uri to recover the row we have inserted the
					// info in and use this for the next insert
					Uri courseRow = cr.insert(
							GradesContract.CONTENT_NAME_COURSES, courseValues);
					Cursor cursor = cr.query(courseRow,
							new String[] { GradesContract.CoursesTable._ID },
							null, null, null);
					cursor.moveToFirst();
					coCode = Integer.toString(cursor.getInt(cursor
					...
					courseValues.put(
							GradesContract.CoursesTable.COL_CO_PASSED_CREDITS,
							plan.getString(nPlanPassedCredits));
					cr.update(GradesContract.CONTENT_NAME_COURSES,
							courseValues, GradesContract.CoursesTable._ID
									+ "=?", new String[] { coCode });
				}
				...
				for (int gradesCounter = 0; gradesCounter < length; gradesCounter++) {
					...
					int subjectId = exists(SUBJECT, subjectName, null, cr,
							language);
					String suCode = null;
					if (subjectId == -1) {
						...
						subjectValues.put(
								...
								grade.getString(nSubjectType));
						subjectValues.put(
								GradesContract.SubjectsTable.COL_SU_CREDITS,
								grade.getString(nSubjectCredits));
						subjectValues.put(
								GradesContract.SubjectsTable.COL_SU_LANGUAGE,
								language);
						// In the same fashion we've done with courses, we need
						// to get the new subject ID
						Uri subjectRow = cr.insert(
								GradesContract.CONTENT_NAME_SUBJECTS,
								subjectValues);
						Cursor cursor = cr
								.query(subjectRow,
										new String[] { GradesContract.SubjectsTable._ID },
										null, null, null);
						cursor.moveToFirst();
						suCode = Integer
								.toString(cursor.getInt(cursor
										.getColumnIndex(GradesContract.SubjectsTable._ID)));
						cursor.close();
					} else {
					...
					if (gradeId == -1) {
						...
						gradeValues.put(
								...
								grade.getString(nGradeName));
						gradeValues.put(
								GradesContract.GradesTable.COL_GR_GRADE,
								grade.getString(nGradeNumber));
						gradeValues.put(
								GradesContract.GradesTable.COL_GR_PASSED,
								grade.getString(nGradePassed));
						gradeValues.put(GradesContract.GradesTable.COL_GR_CODE,
								grade.getString(nGradeLetter));
						gradeValues.put(
								GradesContract.GradesTable.COL_GR_ASSISTED,
								grade.getString(nGradeTaken));
						gradeValues.put(GradesContract.GradesTable.COL_GR_TIME,
								time);
						gradeValues
								.put(GradesContract.GradesTable.COL_GR_REVISION_TIME,
										grade.getString(nGradeRevisionTime));
						...
						gradeValues.put(
								...
								grade.getString(nGradeCallNumber));
						gradeValues.put(
								GradesContract.GradesTable.COL_GR_PROVISIONAL,
								grade.getString(nGradeProvisional));
						gradeValues.put(GradesContract.GradesTable.COL_GR_YEAR,
								grade.getString(nGradeYear));
						gradeValues.put(
								GradesContract.GradesTable.COL_GR_LANGUAGE,
								language);
						cr.insert(GradesContract.CONTENT_NAME_GRADES,
								gradeValues);
					}
				}
			}
	...
	/*
	 * This method will avoid duplicate insertions. Slows down the process but
	 * avoids leaking of storage data.
	 * 
	 * @params
	 * 
	 * @param what - type of row to check
	 * 
	 * @param name - name of the row to check. As the identifier on the database
	 * is an Autoincrement value, it's useless to check on it.
	 * 
	 * @param code - Used when checking grades. we check if the time and subject
	 * to get a unique pair.
	 * 
	 * @param cr - The content resolver to check if the data exists
	 * 
	 * @param language - language of the record
	 * 
	 * @returns int - the number of rows that match the given parameters. 0 if
	 * no row matches. -1 if any error passes by
	 */
	private static int exists(int what, String name, String code,
			ContentResolver cr, String language) {
		int mRowId = -1;
		Cursor cursor = null;
		String selection = null;
		switch (what) {
		case SUBJECT:
			selection = GradesContract.SubjectsTable.COL_SU_NAME + "=? AND "
					+ GradesContract.SubjectsTable.COL_SU_LANGUAGE + "=?";
			String[] subjectSelectionArgs = { name, language };
			cursor = cr.query(GradesContract.CONTENT_NAME_SUBJECTS,
					new String[] { GradesContract.SubjectsTable._ID },
					selection, subjectSelectionArgs, null);
			if (cursor.moveToFirst()) {
				mRowId = cursor.getInt(cursor
						.getColumnIndex(GradesContract.SubjectsTable._ID));
			}
			break;
		case PLAN:
			selection = GradesContract.CoursesTable.COL_CO_NAME + "=? AND "
					+ GradesContract.CoursesTable.COL_CO_LANGUAGE + "=?";
			String[] planSelectionArgs = { name, language };
			cursor = cr.query(GradesContract.CONTENT_NAME_COURSES,
					new String[] { GradesContract.CoursesTable._ID },
					selection, planSelectionArgs, null);
			if (cursor.moveToFirst()) {
				mRowId = cursor.getInt(cursor
						.getColumnIndex(GradesContract.CoursesTable._ID));
			}
			break;
		case STUDENT:
			cursor = cr.query(GradesContract.CONTENT_NAME_STUDENTS,
					new String[] { GradesContract.StudentsTable._ID },
					GradesContract.StudentsTable.COL_ST_NIA + "=\"" + name
							+ "\"", null, null);
			if (cursor.moveToFirst()) {
				mRowId = cursor.getInt(cursor
						.getColumnIndex(GradesContract.StudentsTable._ID));
			}
			break;
		case GRADE:
			selection = GradesContract.GradesTable.COL_GR_TIME + "=? AND "
					+ GradesContract.GradesTable.COL_GR_SU_CODE + "=? AND "
					+ GradesContract.GradesTable.COL_GR_LANGUAGE + "=?";
			String[] gradeSelectionArgs = { name, code, language };
			cursor = cr.query(GradesContract.CONTENT_NAME_GRADES,
					new String[] { GradesContract.GradesTable._ID }, selection,
					gradeSelectionArgs, null);
			if (cursor.moveToFirst()) {
				mRowId = cursor.getInt(cursor
						.getColumnIndex(GradesContract.GradesTable._ID));
			}
			break;
		default:
			return mRowId;
